...
public class Country implements PyEntity {
    ...
    public Country(int code, String name, String locale, int flag,String zg) {
        this.code = code;
        this.name = name;
        this.flag = flag;
        this.locale = locale;
    ...
    public static ArrayList<Country> getAll(@NonNull Context ctx, @Nullable ExceptionCallback callback) {
        ...
        try {
            ...
            for (int i = 0; i < ja.length(); i++) {
                ...
                String zg = jo.getString("zg");
                if(!TextUtils.isEmpty(locale)) {
                    flag = ctx.getResources().getIdentifier("flag_" + locale.toLowerCase(), "drawable", ctx.getPackageName());
                }
                countries.add(new Country(jo.getInt("code"), jo.getString(key), locale, flag,zg));
            }
    ...
    public static Country fromJson(String json){
        ...
        try {
            ...
            return new Country(jo.optInt("code") ,jo.optString("name"), jo.optString("locale"), jo.optInt("flag"),jo.optString("zg"));
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return null;
