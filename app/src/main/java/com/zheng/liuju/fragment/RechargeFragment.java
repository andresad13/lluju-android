...
public class RechargeFragment extends Fragment implements AdapterView.OnItemClickListener, IRecharges , SwipeRefreshLayout.OnRefreshListener, PullToRefreshListView.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        views = inflater.inflate(R.layout.fragment_recharge, container, false);
    ...
    @Override
    public void upData(List<ChongzhiListBean.DataBean> context, int type) {
       /* for (int i = 0; i < 5; i++) {
    ...
    private void more(List<ChongzhiListBean.DataBean> transactionBeans) {
        ...
        if (transactionBeans.size()!=0){
            ...
            for (int i = 0; i < transactionBeans.size(); i++) {
                ...
                if (title.equals("")){
                    TransactionBean transactionBean = new TransactionBean();
                    transactionBean.type = true;
                    transactionBean.dataTitle= transactionBeans.get(i).getAddTime().split(" ")[0];
                    title=transactionBeans.get(i).getAddTime().split(" ")[0];
                    mlist.add(transactionBean);
                }else {
                    if (!transactionBeans.get(i).getAddTime().split(" ")[0].equals(title)){
                        TransactionBean transactionBean = new TransactionBean();
                        transactionBean.type = true;
                        transactionBean.dataTitle= transactionBeans.get(i).getAddTime().split(" ")[0];
                        title=transactionBeans.get(i).getAddTime().split(" ")[0];
                ...
                TransactionBean transactionBean = new TransactionBean();
                if (transactionBeans.get(i).getType().equals("0")){
                ...
                transactionBean.time= transactionBeans.get(i).getAddTime();
                transactionBean.money= "+"+getResources().getString(R.string.moenys)+transactionBeans.get(i).getChongzhi();
                title=transactionBeans.get(i).getAddTime().split(" ")[0];
                mlist.add(transactionBean);
    ...
    private void initData(List<ChongzhiListBean.DataBean> transactionBeans) {
        mlist.clear();
        ...
        if (transactionBeans.size() != 0) {
            ...
            transactionDetailsAdapter = new TransactionDetailsAdapter(getActivity(), mlist,0);
            rentlist.setAdapter(transactionDetailsAdapter);
        } else {
